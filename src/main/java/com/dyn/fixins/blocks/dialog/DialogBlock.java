		...
		if (worldIn.isRemote) {
			if (DYNFixinsMod.accessLevel == PlayerAccessLevel.ADMIN) {
				...
				if (tileentity instanceof DialogBlockTileEntity) {
					DYNFixinsMod.proxy.openEditDialogInterface((DialogBlockTileEntity) tileentity);
				} else {
					return false;
				}
			}
				...
				if ((c1 != null) && (c2 != null)) {
					...
					if (!((DialogBlockTileEntity) tileentity).doesInterrupt()) {
						if (players.contains(Minecraft.getMinecraft().thePlayer)) {
							DYNFixinsMod.proxy.toggleDialogHud(((DialogBlockTileEntity) tileentity).getEntity(), true,
