package com.dyn.fixins.network.messages;
...
public abstract class AbstractMessage<T extends AbstractMessage<T>> implements IMessage, IMessageHandler<T, IMessage> {
	...
	private static <T extends AbstractMessage<T>> void checkThreadAndEnqueue(AbstractMessage<T> msg,
			MessageContext ctx) {
		IThreadListener thread = DYNFixinsMod.proxy.getThreadFromContext(ctx);
		// pretty much copied straight from vanilla code, see {@link
		// PacketThreadUtil#checkThreadAndEnqueue}
		thread.addScheduledTask(() -> msg.process(DYNFixinsMod.proxy.getPlayerEntity(ctx), ctx.side));
	}
	...
	public IMessage onMessage(T msg, MessageContext ctx) {
		if (!msg.isValidOnSide(ctx.side)) {
			...
			msg.process(DYNFixinsMod.proxy.getPlayerEntity(ctx), ctx.side);
		}
		return null;
	}
